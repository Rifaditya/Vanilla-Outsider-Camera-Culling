# ⏱️ Histéresis temporal y cero asignación de memoria (Minecraft 26.3)
...
1. **Parpadeo por roce de cámara (Z-Fighting / Boundary Pop-in)**: Pequeñas rotaciones de la cámara o el movimiento al caminar que crucen el borde de un bloque pueden hacer que las entidades parpadeen rápidamente entre estados visibles y ocluidos en fotogramas alternos.
2. **Tirones por recolección de basura**: La asignación continua de objetos `new Vec3()` y cajas delimitadoras durante el trazado de rayos genera pausas frecuentes del recolector JVM Young-Gen.
...
* **Desoclusión instantánea**: Tan pronto como una sola muestra de rayo confirma línea de visión despejada, la racha de oclusión se elimina (`OCCLUDED_STREAK.remove(id)`), haciendo visible la entidad con latencia de $0\text{ fotogramas}$.
